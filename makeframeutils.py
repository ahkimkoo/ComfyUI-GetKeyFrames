...
def ImgLabeler(img, text, size=72, color=(255,255,255)):
    font = None
    # A list of common font names for cross-platform compatibility
    font_names = [
        "arial.ttf",              # Windows, macOS
        "DejaVuSans.ttf",         # Common on Linux
        "LiberationSans-Regular.ttf" # Another common Linux font
    ]
    ...
    if font is None:
        # If no common fonts are found, fall back to the default bitmap font.
        print(f"Could not find any of {font_names}. Falling back to default font. Text may be very small.")
        font = ImageFont.load_default()

    draw = ImageDraw.Draw(img)

    # Use textbbox for better centering if available (Pillow >= 8.0.0)
    if hasattr(draw, 'textbbox'):
        ...
        # Adjust position based on the bounding box's top-left corner
        text_position = (x - bbox[0], y - bbox[1])
    else: # Fallback for older Pillow versions
        # textsize is deprecated but works as a fallback.
    ...
    # Draw the main text onto the image
